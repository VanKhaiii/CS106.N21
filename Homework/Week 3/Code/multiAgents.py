...
def betterEvaluationFunction(currentGameState):
    ...
    """ Khởi tạo các giá trị đặc trưng """
    closest_food = float('inf')
    closest_scared_ghost = float('inf')
    closest_enemy_ghost = float('inf')


    """ Đặc trưng thứ 1 là số chấm thức ăn còn lại và số viên năng lượng còn lại """
    food_left = len(newFood.asList())
    capsule_left = len(newCapsules)

    """ Đặc trưng thứ 2 là những con ma bình thường và con ma sợ hiền"""
    enemy_ghosts = list()
    scared_ghosts = list()
    ...
    """ Hàm lấy vị trí các con ma trong trạng thái game hiện tại """
    ...
    """ Với con ma Bình Thường thì chúng ta không nên để Pacman lại gần -> đưa Pacman đi xa con ma hung dữ đó
        Xác định khoảng cách gần nhất từ Pacman tới con ma đó, sau đó trừ một lượng nghịch đảo khoảng cách.
        Ta chọn trọng số là 2.0. Vì chúng ta cần minimize giá trị ước lượng nên cần tăng khoảng cách từ Pacman 
        tới con ma đó.
    """
    ...
    """ Với con ma hiền lành thì chúng ta nên để Pacman lại gần để ăn nó, sẽ được lượng điểm lớn,.
        Ta chọn hệ số trừng phạt là 4.0 (ý nghĩa là không khuyến khích Pacman đi xa con mà, thay vào đó đi gần lại sẽ có cơ hội ăn điểm)
    """
    ...
    """ Cho hệ số vào trước đặc trưng cho viên năng lượng và thức ăn còn lại lớn, là 20.0 và 10.0. Ý nghĩa mong muốn Pacman phải cố minimize
    giá trị ước lượng, nên khuyến khích nó nên ăn nhiều viên năng lượng và chấm thức ăn nhất có thể"""
